...
const TextInput = ({ onSubmit }) => {
  ...
  const summarizeText = (text) => {
    const apiKey ="your api key for huggingface.com"// Replace with your Hugging Face API Key
    ...
    axios
        .post(
            modelURL,
            {
                inputs: text,
                parameters: {
                    min_length: 50,        // Minimum length of output in tokens (approx words)
        ...
        .then((response) => {
            // Validate and set the summary
            if (response.data && response.data[0] && response.data[0].generated_text) {
                setSummarizedText(response.data[0].generated_text);
            } else {
  ...
  <div className="container mx-auto mt-10">
    <div className="w-full">
      ...
      >
        <input {...getInputProps()} />
        <span className="text-white text-xl">Drag and drop a .txt or .docx file here, or click to browse.</span>
      </div>
      ...
      {isLoading && <div className="text-center text-md text-gray-600 mt-4">Summarizing... Please wait!</div>}

      {summarizedText && !isLoading && (
        <div className="container mx-auto mt-8 w-full p-4 border-t border-gray-300">
          <h2 className="text-2xl font-extralight tracking-widest text-white mb-2 underline">AI Generated Summarized Text</h2>
